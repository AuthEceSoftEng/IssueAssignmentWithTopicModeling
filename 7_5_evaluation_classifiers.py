...
rects1 = plt.bar(index, [svmMeanAccuracy, svmMeanFmeasure], bar_width, label='SVM', yerr = [svmStdAccuracy, svmStdFmeasure], capsize=3)
rects2 = plt.bar(index + bar_width, [bayesMeanAccuracy, bayesMeanFmeasure], bar_width, label='Naïve Bayes', yerr = [bayesStdAccuracy, bayesStdFmeasure], capsize=3)
ax.set_ylim(0, 1)
#ax.set_xlabel('Metrics')
ax.set_ylabel('Value')
ax.set_xticks(index + bar_width/2, metrics)#, rotation = 45, ha='right')
ax.set_xlim(-0.66, 1.9)
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(resultsdatafolder, "ClassificationModels.eps"))
